...
아래 조건을 **모두** 충족해야 선정한다. 하나라도 미달이면 제외.

1. **독립 개념** — AI 분야에서 독립적으로 정의·설명 가능한 기술·도구·패턴이어야 한다. 특정 제품의 메뉴 옵션 수준이면 제외.
2. **공개 자료 충분** — 공식 문서·논문·기술 블로그 등 신뢰할 수 있는 출처가 **2개 이상** 존재해야 한다. 수집된 소스에서 실질적인 내용을 담은 글이 없고 한 줄 정의밖에 못 쓸 항목은 제외. 조회한 데이터에 해당 키워드에 대한 실질적 설명이 없으면 선정 금지.
3. **커뮤니티에서 실제 화제** — 트렌드/발굴 데이터에 실제로 등장한 것만. 추측·예상·"곧 뜰 것 같다" 근거로 선정 금지.
4. **중복 아님** — 기존 키워드 목록과 의미상 동일한 개념이면 제외 (명칭만 다른 경우 포함).
5. **콘텐츠 생성 가능** — 해당 키워드로 개념 설명·사용 예시·심화 내용 등 3개 이상의 <h4> 섹션을 약 900자 이상의 줄글로 채울 수 있을 만큼 정보가 충분해야 한다. 그렇지 않으면 선정하지 말 것.
